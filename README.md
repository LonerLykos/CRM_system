...
A CRM for processing course-enrollment requests ("orders"): managers work through incoming
orders, leave comments and export filtered lists to Excel, while admins manage manager
accounts and view aggregated statistics.
 
Monorepo of two apps — a Django + DRF REST API and a Next.js web client — orchestrated with
Docker Compose.
 
**Highlights:** JWT (RS256) in httpOnly cookies with refresh and blacklisting · role-based
permissions (admin / manager) · filtering, ordering and pagination on the order list ·
hybrid sync/async Excel export backed by Celery · health probe and healthcheck-based startup
ordering · separate development and production Compose stacks · pytest + vitest.
 
---
 
## Table of contents
 
- [A note on the committed `.env` and keys](#a-note-on-the-committed-env-and-keys)
- [Stack](#stack)
...
## A note on the committed `.env` and keys
 
This is a demo / portfolio project, and the `.env` file together with the RSA key pair in
`backend/extra_data/keys/` are committed **on purpose**, so that the repository can be cloned
and started with a single command instead of a manual setup ritual.
 
All of these values are throwaway: they were generated for this demo only, are not reused
anywhere else, and are meaningless outside a local environment. The seeded admin account
(`admin@gmail.com` / `admin`) exists for the same reason.
 
In a real deployment none of this belongs in version control: `.env` and the key directory
would be gitignored (`.env.example` lists every variable that is required), secrets would be
injected from the environment or a secret manager, and the JWT key pair would be generated
and rotated per environment.
...
- A `.env` file in the repository root (already committed for the demo — see the note above;
  otherwise copy it from `.env.example` and fill in the values).
- Access to an external MySQL server — the stack does **not** run a local MySQL container.
## Configuration
 
If you are starting from a clean checkout of your own, copy the template and fill in the values:
...
| `REDIS_URL`                | Redis URL for the Celery broker/result backend (e.g. `redis://redis:6379/0`) |
...
Uses `docker-compose.yml` (base) plus `docker-compose.override.yml` (dev frontend), which
...
The `crm` service has a Docker healthcheck probing `GET /health`, so `docker compose ps`
reports its health; the frontend waits for `crm` to become healthy before starting.
 
### Production
 
Uses `docker-compose.yml` (base) plus `docker-compose.prod.yml`, which builds the frontend
from `Dockerfile_prod` (multi-stage standalone build, `NODE_ENV=production`).
...
Database migrations run automatically on backend startup (`migrate --noinput`), after
`wait_db` blocks until MySQL is reachable. The following management commands seed data and
are idempotent:
 
```bash
# Default admin: admin@gmail.com / admin  (demo seed account — see the note above)
...
Authentication is JWT (RS256) delivered via httpOnly cookies (`access_token`,
`refresh_token`); the login and set-password responses also return the tokens in the body.
A middleware copies the access-token cookie into the `Authorization` header for DRF.
...
A new manager is created by an admin via `POST /users/create_user`; the response returns a
one-time link the manager uses to set a password.
...
`GET /orders/export` exports the currently filtered order list (same `OrderFilter` as
`GET /orders`, no pagination) to `.xlsx`. It is a hybrid endpoint:
 
- if the row count is `≤ EXPORT_SYNC_MAX_ROWS` (default **1000**), the file is built
  synchronously and returned inline (`200`);
- otherwise a Celery task is dispatched (`202 {task_id}`); poll
  `GET /orders/export/{task_id}` for state/progress, then download via
  `GET /orders/export/{task_id}/download`.
Generated files live in the `exports_media` volume and are purged hourly by a Celery-beat
task (1-hour TTL). The Next.js proxy at `/api/orders/export` hides the sync/async split from
the browser.
...
│       └── keys/                 # RSA keys for JWT (demo only — see the note above)
├── frontend/
│   ├── Dockerfile                # dev
...
(`base_url`, credentials, `order_id`, `user_id`, `task_id`, `set_password_token`) are
collection-scoped, so no external environment is needed — override `base_url` in the
collection's **Variables** tab if required.
